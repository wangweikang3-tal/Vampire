...
//ui拓展类，可以拓展UI的功能
    public static class UIExtends
    {
        ...
        //拓展方法，必须要Static，第一次参数必须是this开头，this后面的是类型，表示这个类型的实例可以拓展方法
        ...
        public static void AddClick(this Button button, Action<object> call, object state)
        {
            ...
            button.onClick.AddListener(() => { call?.Invoke(state); });
        }

        public static Component FindChild(this ScriptWindow w, string type, string name)
        {
            switch (type)
            {
        ...
        public static Component Find(this ScriptWindow w, string type, string path)
        {
            switch (type)
            {
